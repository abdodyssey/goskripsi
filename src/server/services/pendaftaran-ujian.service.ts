...
async function uploadToSupabase(
  ...
  const ext = rawExt.replace(/[^a-zA-Z0-9.]/g, "").toLowerCase().substring(0, 10) || ".pdf";
...
export class PendaftaranUjianService {
  ...
  async store(payload: CreatePendaftaranUjianInput, files: UploadFile[]) {
    ...
    if (Number(mahasiswa.ipk) < 2) {
      throw new HttpError(400, `Syarat IPK minimal 2.0. IPK Anda saat ini: ${mahasiswa.ipk}`);
    }

    if (mahasiswa.semester < 6) {
      throw new HttpError(400, `Syarat semester minimal semester 6. Semester Anda saat ini: ${mahasiswa.semester}`);
    }
    ...
    if (targetJenisId === 2) {
      const hasPassedSempro = await prisma.ujian.findFirst({
        where: {
          pendaftaranUjian: { mahasiswaId: Number(payload.mahasiswa_id), jenisUjianId: 1 },
          hasil: "lulus",
        },
      });
      if (!hasPassedSempro) throw new HttpError(400, "Harus lulus Seminar Proposal dahulu.");
    }

    if (targetJenisId === 3) {
      const hasPassedHasil = await prisma.ujian.findFirst({
        where: {
          pendaftaranUjian: { mahasiswaId: Number(payload.mahasiswa_id), jenisUjianId: 2 },
          hasil: "lulus",
        },
      });
      if (!hasPassedHasil) throw new HttpError(400, "Harus lulus Ujian Hasil dahulu.");
    }
    ...
    return await prisma.$transaction(async (tx) => {
      ...
      if (uploadedFiles.length > 0) {
        const allSyarat = await tx.syarat.findMany({ where: { jenisUjianId: Number(payload.jenis_ujian_id) } });
        for (const uf of uploadedFiles) {
          const matchingSyarat = allSyarat.find((s) => {
            const lastDotIndex = uf.nama_berkas.lastIndexOf(".");
            const nameOnly = lastDotIndex !== -1 ? uf.nama_berkas.substring(0, lastDotIndex) : uf.nama_berkas;
            return s.namaSyarat.trim().toLowerCase() === nameOnly.trim().toLowerCase();
          });
          if (matchingSyarat) {
            await tx.pemenuhanSyarat.upsert({
              where: { pendaftaranUjianId_syaratId: { pendaftaranUjianId: pendaftaran.id, syaratId: matchingSyarat.id } },
              create: { pendaftaranUjianId: pendaftaran.id, syaratId: matchingSyarat.id, fileBukti: uf.file_path, terpenuhi: true },
  ...
  async update(id: string, payload: UpdatePendaftaranUjianInput, files: UploadFile[]) {
    ...
    return await prisma.$transaction(async (tx) => {
      ...
      if (payload.ranpel_id) {
        const approvedRanpel = await tx.pengajuanRancanganPenelitian.findFirst({
          where: { rancanganPenelitianId: Number(payload.ranpel_id), mahasiswaId: pendaftaran.mahasiswaId, statusKaprodi: "diterima" },
        });
        if (!approvedRanpel) throw new HttpError(400, "Ranpel belum disetujui.");
        dataUpdate.rancanganPenelitianId = Number(payload.ranpel_id);
      }
      if (payload.jenis_ujian_id) dataUpdate.jenisUjianId = Number(payload.jenis_ujian_id);
      if (payload.status === "diterima" && !pendaftaran.tanggalDisetujui) dataUpdate.tanggalDisetujui = new Date();

      await tx.pendaftaranUjian.update({ where: { id: Number(id) }, data: dataUpdate });

      if (uploadedFiles.length > 0) {
        const allSyarat = await tx.syarat.findMany({ where: { jenisUjianId: Number(payload.jenis_ujian_id || pendaftaran.jenisUjianId) } });
        for (const uf of uploadedFiles) {
          const matchingSyarat = allSyarat.find((s) => {
            const lastDotIndex = uf.nama_berkas.lastIndexOf(".");
            const nameOnly = lastDotIndex !== -1 ? uf.nama_berkas.substring(0, lastDotIndex) : uf.nama_berkas;
          ...
          if (matchingSyarat) {
            await tx.pemenuhanSyarat.upsert({
              where: { pendaftaranUjianId_syaratId: { pendaftaranUjianId: Number(id), syaratId: matchingSyarat.id } },
              create: { pendaftaranUjianId: Number(id), syaratId: matchingSyarat.id, fileBukti: uf.file_path, terpenuhi: true },
              update: { fileBukti: uf.file_path },
            });
          }
        }
  ...
  async submit(id: string) {
    const pendaftaran = await prisma.pendaftaranUjian.findUnique({
      where: { id: Number(id) },
      include: { jenisUjian: { include: { syarats: { where: { wajib: true } } } }, pemenuhanSyarats: true },
    });
    if (!pendaftaran) throw new HttpError(404, "Data pendaftaran tidak ditemukan");
    if (pendaftaran.status !== "revisi") throw new HttpError(400, "Status harus revisi.");
    ...
    if (missingSyarat.length > 0) throw new HttpError(400, "Syarat wajib belum lengkap.");
  ...
  async review(id: string, status: "revisi" | "diterima" | "ditolak", keterangan?: string) {
    const data: any = { status };
    if (keterangan) data.keterangan = keterangan;
    if (status === "diterima") data.tanggalDisetujui = new Date();

    const updated = await prisma.pendaftaranUjian.update({ where: { id: Number(id) }, data, select: PENDAFTARAN_SELECT });
    return this.transformPendaftaran(updated);
  }

  async uploadRevisi(id: string, files: UploadFile[]) {
    const pendaftaran = await prisma.pendaftaranUjian.findUnique({ where: { id: Number(id) }, include: { mahasiswa: true } });
    if (!pendaftaran) throw new Error("Data pendaftaran tidak ada");
    ...
    return await prisma.$transaction(async (tx) => {
      const allSyarat = await tx.syarat.findMany({ where: { jenisUjianId: pendaftaran.jenisUjianId } });
      for (const uf of uploadedFiles) {
        const matchingSyarat = allSyarat.find((s) => {
          const lastDotIndex = uf.nama_berkas.lastIndexOf(".");
          const nameOnly = lastDotIndex !== -1 ? uf.nama_berkas.substring(0, lastDotIndex) : uf.nama_berkas;
      ...
      return await tx.pendaftaranUjian.update({ where: { id: Number(id) }, data: { status: "menunggu", keterangan: null }, select: PENDAFTARAN_SELECT });
    });
  }

  async delete(id: string) {
    return await prisma.$transaction(async (tx) => {
      await tx.pemenuhanSyarat.deleteMany({ where: { pendaftaranUjianId: Number(id) } });
      return await tx.pendaftaranUjian.delete({ where: { id: Number(id) } });
    });
  }

  async getByMahasiswa(params: { mahasiswaId: string; skip?: number; take?: number }) {
    ...
    return { data: list.map((p) => this.transformPendaftaran(p)), meta: createPaginationMeta(total, Math.floor(skip / take) + 1, take) };
  }
  ...
  private transformPendaftaran(p: any) {
    ...
    return {
      ...
      mahasiswa: p.mahasiswa ? { ...p.mahasiswa, nama: p.mahasiswa.user?.nama, ipk: p.mahasiswa.ipk ? Number(p.mahasiswa.ipk) : 0 } : undefined,
      rancanganPenelitian: p.rancanganPenelitian,
      jenisUjian: p.jenisUjian,
      berkas: (p.pemenuhanSyarats || []).map((ps: any) => ({ id: ps.id, namaBerkas: ps.syarat?.namaSyarat || "Berkas", filePath: ps.fileBukti })),
      ujian: p.ujian,
    };
  }
}
