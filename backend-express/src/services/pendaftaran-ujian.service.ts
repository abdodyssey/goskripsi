...
async function uploadToSupabase(
  ...
  console.log(
    `[Upload] Uploading to Supabase: ${storagePath} (${file.mimetype}, ${file.size} bytes)`,
  );
...
export class PendaftaranUjianService {
  async getAll() {
    const list = await prisma.pendaftaranUjian.findMany({
      include: {
  ...
  async store(
    payload: CreatePendaftaranUjianInput,
    ...
    if (targetJenisId === 2) {
      ...
      if (!hasPassedSempro) {
        throw new HttpError(
          400,
          "Anda harus lulus Seminar Proposal terlebih dahulu sebelum mendaftar Ujian Hasil",
        );
      }
    }
    ...
    if (targetJenisId === 3) {
      ...
      if (!hasPassedHasil) {
        throw new HttpError(
          400,
          "Anda harus lulus Ujian Hasil terlebih dahulu sebelum mendaftar Ujian Skripsi",
        );
      }
    }
  ...
  async update(
    ...
    const nim =
      (pendaftaran as any).mahasiswa?.nim || pendaftaran.mahasiswaId.toString();
  ...
  async submit(id: string) {
    ...
    if (pendaftaran.status !== "draft" && pendaftaran.status !== "revisi") {
      throw new HttpError(
        400,
        "Pendaftaran hanya bisa di-submit jika status draft atau revisi",
      );
    }
    ...
    if (missingSyarat.length > 0) {
      ...
      throw new HttpError(400, `Syarat wajib belum lengkap: ${names}`);
    }
  ...
  async uploadRevisi(id: string, files: Express.Multer.File[]) {
    const pendaftaran = await prisma.pendaftaranUjian.findUnique({
      where: { id: Number(id) },
      include: { mahasiswa: true },
    });
    ...
    if (pendaftaran.status !== "revisi") {
      throw new Error("Hanya bisa upload ulang jika status revisi");
    }
  ...
  async getByMahasiswa(mahasiswaId: string) {
    const list = await prisma.pendaftaranUjian.findMany({
