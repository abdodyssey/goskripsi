...
class MailService {
     ...
     */
    async sendRanpelSubmissionNotification(dosenEmail, dosenNama, studentNama, judul) {
     ...
     */
    async sendRanpelReviewNotification(studentEmail, studentNama, reviewerNama, status, catatan) {
        if (!process.env.RESEND_API_KEY)
            return;
        try {
