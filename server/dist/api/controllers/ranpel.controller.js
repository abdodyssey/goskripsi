...
class RanpelController {
    ...
    async exportPdf(req, res, next) {
        try {
            ...
            const dataToPdf = {
                ...
                tanggalReviewPa: pengajuan.tanggalReviewPa,
                tanggalReviewKaprodi: pengajuan.tanggalReviewKaprodi,
                prodiName: pengajuan.mahasiswa?.prodi?.namaProdi,
                statusKaprodi: pengajuan.statusKaprodi,
            };
    ...
    async exportSuratJudul(req, res, next) {
        try {
            const filePath = path_1.default.join(process.cwd(), "pengajuan judul dan pembimbing.pdf");
            if (!fs_1.default.existsSync(filePath)) {
                return res.status(404).json({ message: "File template not found" });
            }
            res.download(filePath, "SURAT_PENGAJUAN_JUDUL.pdf");
        }
        catch (error) {
            next(error);
        }
