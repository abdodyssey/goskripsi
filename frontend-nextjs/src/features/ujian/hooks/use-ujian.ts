...
export const useUjian = () => {
  ...
  const createSchedulingMutation = useMutation({
    mutationFn: (payload: any) => ujianService.createScheduling(payload),
    onSuccess: () => {
  ...
  const submitAbsensiMutation = useMutation({
    mutationFn: ({ id, absensiList }: { id: string; absensiList: any[] }) =>
  ...
  const simpanDraftNilaiMutation = useMutation({
    mutationFn: ({ id, penilaianList }: { id: string; penilaianList: any[] }) =>
